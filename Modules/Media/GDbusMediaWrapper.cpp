    ...
    if (session_bus) {
        ...
        if (reply) {
            ...
            if (type == G_DBUS_MESSAGE_TYPE_ERROR) {
                ...
                g_autoptr(GVariant) v = g_dbus_message_get_body(reply);
                GVariantIter *iter;
                gchar *str;
...
bool GDbusMediaWrapper::sendMediaPlayerCommand(const std::string &dbus_player, const std::string &command) {
    ...
    GDBusMessage *msg_command = g_dbus_message_new_method_call(dbus_player.c_str(),
                                                                         "/org/mpris/MediaPlayer2",
                                                                         "org.mpris.MediaPlayer2.Player",
                                                                         command.c_str());  // PlayPause | Next | Previous

    auto result = g_dbus_connection_send_message(  session_bus,
                                                   msg_command,
                                                   G_DBUS_SEND_MESSAGE_FLAGS_NONE,
                                                   nullptr,
                                                   nullptr);
        ...
        if (type == G_DBUS_MESSAGE_TYPE_ERROR) {
            ...
            switch (var_type) {
                case G_VARIANT_CLASS_STRING:
                    ...
                    return std::make_any<std::string>(str_out);
                case G_VARIANT_CLASS_BOOLEAN:
                    gboolean bool_out;
                    g_variant_get(child, "b", &bool_out);
                    return std::make_any<bool>(bool_out);
                default:
                    return {};
            }
        }
