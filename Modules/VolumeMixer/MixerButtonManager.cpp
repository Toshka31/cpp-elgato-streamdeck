...
std::shared_ptr<MixerButtonManager> MixerButtonManager::getManager(const std::shared_ptr<IDeviceButtonRestricted>& device) {
...
std::vector<unsigned char> MixerButtonManager::getButtonImage(unsigned short key) {
    if (key == BUTTON_HOME_INDEX)
        ...
        && m_key_apps.find(key) != m_key_apps.end())
        return IMAGE_PLUS;
    else if (key >= BUTTON_VOLUME_DOWN_FIRST_COLUMN_INDEX && key <= BUTTON_VOLUME_DOWN_LAST_COLUMN_INDEX
        && m_key_apps.find(key % BUTTONS_IN_ROW) != m_key_apps.end())
        return IMAGE_MINUS;
    else if (key >= BUTTON_APP_FIRST_COLUMN_INDEX && key <= BUTTON_APP_LAST_COLUMN_INDEX
        && m_key_apps.find(key % BUTTONS_IN_ROW) != m_key_apps.end())
